...
if [ $# -ne 3 ]; then
    echo "Usage: $0 <Common_voice_base_dir> <lang> <norm_type: tc, lc.rm>"
    echo "  lang: en de fr cy tt kab ca zh-TW it fa eu es ru tr nl eo zh-CN rw pt zh-HK cs pl uk"
    exit;
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for part in "validated" "test" "dev"; do
        ...
        local/data_prep.pl "${COMMONVOICE}/cv-corpus-8.0-2022-01-19/${lang}" ${part} data/"$(echo "${part}_${lang}" | tr - _)"
    done
