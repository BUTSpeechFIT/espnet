...
stop_stage=2
SECONDS=0
lang=it # en de fr cy tt kab ca zh-TW it fa eu es ru tr nl eo zh-CN rw pt zh-HK cs pl uk
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    log "stage 1: Preparing data for commonvoice for ${lang} language"
...
#        local/data_prep.pl "/mnt/matylda2/data/mozilla_common_voice-8.0-2022-01-19/cv-corpus-8.0-2022-01-19/${lang}" ${part} data/"$(echo "${part}_${lang}" | tr - _)"
        local/data_prep.pl "/mnt/matylda4/xsarva00/datasets/mozilla_common_voice-8.0-2022-01-19/cv-corpus-8.0-2022-01-19/${lang}" ${part} data/"$(echo "${part}_${lang}" | tr - _)"
    done
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    log "stage 2: Normalizing data for commonvoice for ${lang} language"
    # normalize
    for part in "train" "test" "dev"; do
      src=data/$(echo "${part}_${lang}" | tr - _)
      ...
      tokenizer.perl -l ${lang} -q < ${dst}/${lang}.norm > ${dst}/${lang}.norm.tc.tok
      awk '{print $1}' ${dst}/text > ${dst}/id
      paste -d " " ${dst}/id ${dst}/${lang}.norm.tc.tok | sort > ${dst}/text.tc.${lang}
      cp ${dst}/text.tc.${lang} $src/text
    done
