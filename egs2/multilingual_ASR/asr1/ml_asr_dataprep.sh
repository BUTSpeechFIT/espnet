...
# TODO: rewrite token_listdir as argument into this script
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        for l in $langs; do
            local/data_tom.sh ${local_data_opts} --lang $l

            if  [ "${feats_type}" = extracted ] || [ ! -z ${hours} ]; then
                # create fbanks for each language and set
                ./get_fbank_commonvoice.sh $l
            fi

            if [ ! -z ${hours} ]; then
                ## compute lengths so X hours can be taken from whole dataset
                utils/data/get_utt2dur.sh data/${train_set}_${l}

                ## prepare new train folder for X hours of data
                new_train_dir="${train_set}_${l}_${hours}"
                utils/fix_data_dir.sh data/${train_set}_${l}
                cp -r data/${train_set}_${l} data/$new_train_dir
                
                # cut only ${hours} hours of data (feats.scp and wav.scp) based on content of utt_${hours} file
                python3 get_Xhours_commonvoice.py --hours ${hours}  --datadir data/${new_train_dir}
                grep -F -f data/${new_train_dir}/utt_${hours} data/${new_train_dir}/feats.scp > data/${new_train_dir}/tmp_feats
                cat data/${new_train_dir}/tmp_feats > data/${new_train_dir}/feats.scp

                grep -F -f data/${new_train_dir}/utt_${hours} data/${new_train_dir}/wav.scp > data/${new_train_dir}/tmp_wav
                cat data/${new_train_dir}/tmp_wav > data/${new_train_dir}/wav.scp
                utils/fix_data_dir.sh data/${new_train_dir}
            fi
        done
    fi
    
    # create train, val and test directiories containing datat for selected languages
    if [ ${create_dataset} = true ]; then
        log "Creating dataset from ${langs} language datasets."
        # Combined train and valid datasets for each languages in $langs into one dataset.
        # Create list of test sets (one test set for each language), 
        # do not combine test sets.
        # Set variables to correct path to new dset directiories

        # sort languages so the same dataset isn't present multiple times
        langs_dataset=$(echo ${langs} | tr " " "\n" | sort -g | tr "\n" "_")
        new_train_set_appendix=_${langs_dataset}${hours}
        new_valid_set_appendix=_${langs_dataset::-1} # remove last '_'
            
        train_list="" 
        valid_list="" 
        test_list="" 
        for ll in ${langs}; do
            # combine data of all given languages into train,test and validation datasets
            lang_train="data/${train_set}_${ll}_${hours}"
            lang_valid="data/${valid_set}_${ll}"
            lang_test="${test_sets}_${ll}"
            train_list="${train_list} ${lang_train}" 
            valid_list="${valid_list} ${lang_valid}" 
            test_list="${test_list} ${lang_test}" 
        done

        combine_data.sh data/${train_set}${new_train_set_appendix} "${train_list}"
        combine_data.sh data/${valid_set}${new_valid_set_appendix} "${valid_list}"
        #combine_data.sh data/${test_sets}${new_train_set_appendix} "${test_list}"

        utils/fix_data_dir.sh data/${train_set}${new_train_set_appendix}
        utils/fix_data_dir.sh data/${valid_set}${new_valid_set_appendix}
        #utils/fix_data_dir.sh data/${test_sets}${new_train_set_appendix}
        ...
        # no need to running "Build dset names" stage
        build_dset_names=false
    fi 

    if [ ${build_dset_names} = true ]; then
        log "Building dset names from ${langs} languages"
